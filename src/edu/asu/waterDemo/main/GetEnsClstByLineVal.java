...
@Path("/getEnsClstByLineVal")
public class GetEnsClstByLineVal {
...
//			the color transfer function should be a string concat by '&' and each part should be in the form of  R,G,B
			...
			@QueryParam("model") @DefaultValue("null") String model,
		...
		files = getAllFiles(baseDir, files, model);
		ArrayList<TiffParser> parsers = new ArrayList<TiffParser>();
		parsers = loadTiffFiles(files);
		String targetPath = baseDir + model +  "_" + minmaxType + ".png";
		...
		for(int i=0; i<NUMBER_OF_PROCESSORS; i++){
			...
			drawOverviewMapService[i] = new DrawMapThread(startIndex, endIndex, minmaxType, parsers, tile);
